...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Paris Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  Paris-cli [options] <command> [params]  " + _("Send command to Paris Core") + "\n" +
                  "  Paris-cli [options] help                " + _("List commands") + "\n" +
                  "  Paris-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const string& strMethod, const UniValue& params)
{
    ...
    // Paris:
    // we get Parisd output (possibly binary) of .write() that either:
