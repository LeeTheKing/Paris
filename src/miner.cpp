...
CBlockTemplate* CreateNewBlockInner(const CScript& scriptPubKeyIn, bool fAddProofOfStake, bool& fPoSCancel, CWallet* pwallet)
{
    ...
    if (fAddProofOfStake)  // attemp to find a coinstake
    {
        ...
        if (fPoSCancel)
            return NULL; // Paris: there is no point to continue if we failed to create coinstake
    }
    else
...

    {
        ...
        CValidationState state;
        if (!TestBlockValidity(state, *pblock, pindexPrev, false, false, false)) // Paris: we do not check block signature here, since we did not sign it yet
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ParisMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ParisMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread(fProofOfStake? "Paris-stake-minter" : "Paris-miner");
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in ParisMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("Running ParisMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ParisMiner terminated\n");
	return;
        // throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ParisMiner runtime error: %s\n", e.what());
        return;
    }
}
